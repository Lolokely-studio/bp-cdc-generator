...
function Drafting({ state, live, catalogue }: { state: ProjectState; live: Live; catalogue: Catalogue }) {
  ...
  const starting = state.sections.length === 0 && Object.keys(state.faits).length === 0 && !live.draft;
  let label: string;
  if (live.rework !== null) {
...
- [ ] **Étape 1 : la configuration de Playwright**
...
- [ ] **Étape 3 : les deux écrans que le parcours ne montre pas**
...
- [ ] **Étape 4 : l'intégration continue**
...
- [ ] **Étape 7 : la suite complète des deux côtés**
