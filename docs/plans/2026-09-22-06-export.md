...
Deux décisions de conception, prises par la relecture et retenues telles
quelles, puis une série de trous de tests.

**1. Un export raté était invisible.** `export_project` attrapait l'erreur et
se contentait de la journaliser. Le front voyait `en_cours` repasser à faux,
avec soit rien, soit les fichiers de l'export précédent, qui avaient l'air d'un
succès. C'est l'inverse du §7 : dire plutôt que masquer. Chaque export laisse
désormais son issue en mémoire, et `GET` la rend sous `dernier_export`. Elle se
perd au redémarrage, comme `en_cours` ; c'est acceptable.

**2. Un export partiel mélangeait l'ancien et le nouveau.** Les fichiers étaient
rendus, déposés et enregistrés un par un. Un échec au PDF du second document
laissait des fichiers neufs à côté de fichiers anciens ; dans le pire cas, le
Word neuf et le PDF ancien d'un même document, au contenu contradictoire.
L'ordre devient : **tout rendre**, puis **tout déposer**, puis **tout
enregistrer en une transaction**. Un échec au rendu, le plus probable, laisse
l'export précédent intact, en stockage comme en base. `GET` rend aussi
`cree_le` pour chaque fichier.

**3. La base ne protégeait pas l'unicité.** Le registre en mémoire empêche deux
exports simultanés dans un processus, mais rien en base. La migration `0005`
ajoute un index unique sur `(project_id, document, format)`, et l'écriture
devient un `insert … on conflict do update`, sans suppression préalable.

La migration `0005` n'a jamais touché la base réelle : elle est réécrite, pas
complétée par une `0006`.

```python
# backend/migrations/versions/0005_exports_fidele.py  — upgrade complet
def upgrade() -> None:
    op.execute(
        "alter table exports add column fidele boolean not null default true")
...
Dans `app/projects/repository.py`, `record_export` est remplacée par :
...
La route `listing` rend, en plus de `en_cours` et `fichiers` :
`"dernier_export": last_outcome(str(project_id))`, et pour chaque fichier
`"cree_le": row["created_at"].isoformat()`.

**Un avis, retenu sans code :** un graphique construit sur des calculs plus
récents que le tableau d'une section rouverte peut contredire ce tableau. La
section n'étant pas `done`, le document est déjà un brouillon ; c'est
acceptable ici, à revoir avec la réécriture des sections rouvertes.

Tests à ajouter ou à corriger dans `backend/tests/test_export_routes.py`.
D'abord une attente sur la fin réelle de l'export, à la place des
`asyncio.sleep(1)`, qui laissaient survivre une mutation dès que le rendu
...
Puis ces tests, qui remplacent `test_a_failed_upload_records_nothing` et
`test_a_failed_rendering_records_nothing` :
...
Et dans `backend/tests/test_lifespan.py`, le câblage de l'arrêt, qui n'était
tenu par rien :
...
Vérifiez comment `test_lifespan.py` entre déjà dans `main.lifespan` et
alignez-vous dessus. Si `main` importe `close_checkpointer` sous un autre nom,
patchez le nom que le cycle de vie appelle réellement.

| Mutation | Doit faire tomber |
|---|---|
| enregistrer avant de déposer | `..._failed_upload_records_nothing`, par son assertion |
| déposer au fil du rendu, document par document | `..._failed_export_leaves_the_previous_one_untouched` |
| ne pas noter l'issue en cas d'échec | `..._failed_rendering_records_nothing` |
| retirer le `try/except` | le même |
| retirer l'index unique ou le `on conflict` | `..._exporting_twice_keeps_one_row_per_file` |
| types de contenu du Word et du PDF inversés | `..._stored_with_their_content_type` |
| `signed_url` sur un chemin fixe | `..._each_link_signs_its_own_file` |
| `brouillon` figé à vrai | `..._finished_project_is_not_a_draft` |
| `_computations` rend `{}` | `..._business_plan_export_embeds_its_charts` |
| export `cancel_all` retiré du cycle de vie | `..._cancels_exports_before_closing_the_checkpointer` |

**Trois tests de ce tableau étaient encore aveugles, trouvés en l'exécutant.**
« Déposer document par document » ne pouvait pas se voir sur un projet à un
seul document : un test sur un projet `both`, dont le second document échoue
au rendu, vérifie qu'aucun dépôt n'a lieu. Retirer le `on conflict` passait,
parce que l'annulation de la transaction préservait les lignes précédentes et
que le test ne faisait que les compter : il vérifie aussi `dernier_export`
à chaque tour. Et le corps de `_computations` n'était exécuté par aucun test,
celui des graphiques remplaçant la fonction entière :
`test_computations_are_read_from_the_checkpoint` lui fournit un graphe dont
l'état porte des calculs.
...
Une exécution réelle d'un projet `both` jusqu'au bout, point de reprise purgé,
puis l'export réel : 29 sections `done`, les dix calculs survivent à la purge,
deux graphiques dans le Word du business plan, tableaux numérotés de 1 à 10,
titre « Coach & <Dom> » échappé. L'authentification de Gotenberg a été vérifiée
sur la vraie image : 401 sans identifiants, 401 avec un mauvais mot de passe.

Deux tests ajoutés avant la fusion : l'argument de `_computations` est bien le
fil du projet (sinon plus aucun graphique, en silence), et une réexportation
après la fin de la rédaction efface `brouillon`.

**Une instabilité observée, non reproduite.** Ce second test a échoué deux fois
dans la suite complète, puis passé seize suites complètes et vingt-cinq
passages ciblés d'affilée. La cause n'est pas établie. Le test vérifie
désormais chaque étape — code de retour du second lancement, issue de l'export
— et imprime l'état complet s'il échoue, pour que la prochaine occurrence dise
elle-même ce qui s'est passé.

**Reporté, avec la raison :** les numéros de tableau et les graphiques ne sont
vérifiés que dans le document neutre, pas dans le Word ni le PDF rendus ;
l'atomicité de `record_exports` n'est pas testée ; un lien refusé par le
stockage fait échouer toute la liste en 500 ; `app.main` importe au démarrage
matplotlib, xhtml2pdf et docxtpl, soit environ 50 Mo sur 512, qu'un import
paresseux éviterait.
