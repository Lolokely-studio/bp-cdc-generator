...
**Prérequis du propriétaire, à fournir avant la tâche 5 :** dans Supabase,
créer un bucket **privé** nommé `exports`, et placer dans `backend/.env` les
variables `SUPABASE_URL` (l'URL du projet) et `SUPABASE_SERVICE_ROLE_KEY`. La
clé de service ne sort jamais du serveur. Les tâches 1 à 4 ne dépendent pas de
ce prérequis.
