...
**Découverte pendant la tâche 4, reproduite par le contrôleur. Correction du plan 1, pas du plan 3 — mais elle ne peut pas attendre.**

**Fichiers :**
- Modifier : `backend/app/core/db.py`, `backend/app/main.py`
- Test : `backend/tests/test_db.py`

**Le défaut.** `connection()` appelle `await pool.open(wait=True)` à chaque usage. `wait=True` attend que le pool soit prêt, c'est-à-dire qu'il ait `min_size` connexions **disponibles**. Dès qu'un appelant en tient une, le suivant attend une disponibilité que le premier ne rendra qu'après lui — blocage, alors que `max_size` vaut cinq. Reproduit :
...
**Pourquoi `wait=True` était là.** Le plan 1 l'a ajouté contre une course réelle et mesurée : sans lui, la toute première connexion échouait ou restait bloquée environ une fois sur deux, parce que `open()` rend la main avant que le remplissage initial ne soit terminé. La correction doit garder cette protection, pas l'échanger contre l'ancien défaut.

**La correction : attendre une fois, pas à chaque appel.**
...
async def connection():
    ...
    `wait` attend que le pool ait `min_size` connexions disponibles, or un
    appelant qui en tient déjà une attend alors une disponibilité qu'il est
    seul à pouvoir rendre. Deux connexions simultanées se bloquent, malgré un
    `max_size` de cinq. On attend donc une fois, sous verrou, et plus jamais.
    """
    global _ready
...
async def close_pool() -> None:
    """Ferme le pool et oublie qu'il était prêt : sans cette remise à zéro, un
    cycle de vie redémarré dans le même processus croirait le pool ouvert."""
    global _ready
    await pool().close()
    _ready = False
```

`app/main.py` appelle `close_pool()` là où il fermait le pool directement, en gardant l'imbrication des `finally` déjà en place.
...
async def test_two_connections_can_be_held_at_once(migrated_db):
    """Le défaut d'origine : `open(wait=True)` à chaque appel faisait attendre
    au second appelant une disponibilité que le premier ne rendait qu'après
    lui. Huit secondes suffisent largement — le blocage était infini."""
    import asyncio
...
**Prouver l'échec avant la correction.** Lancer ce test contre le `connection()` actuel et constater le dépassement de délai, puis appliquer la correction et le voir passer. Reporter les deux sorties.

**Ce que cette correction ne fait pas.** Elle ne touche ni `min_size` ni `max_size`, et ne change rien au mode transaction du pooler.
