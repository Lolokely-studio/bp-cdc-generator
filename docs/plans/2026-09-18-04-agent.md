...
- [ ] **Étape 6 : écrire `app/agent/state.py`**
...
class EsquisseState(TypedDict):
    ...
    idea: str
    # Le seul champ réduit : voir `app.agent.facts.merge_facts`.
...
**Attention à l'annotation du réducteur.** Le bloc ci-dessus met une chaîne en place du réducteur pour éviter un import circulaire entre `state` et `facts`. **Ce n'est pas la forme finale** : LangGraph exige la fonction elle-même. L'implémenteur remplace `Annotated[dict[str, Fact], "merge_facts"]` par `Annotated[dict[str, Fact], merge_facts]` et importe `merge_facts` depuis `app.agent.facts` — l'import va de `state` vers `facts`, et `facts` n'importe de `state` que `Fact`, ce qui ne boucle pas si `facts` est écrit d'abord. **Si un cycle apparaît malgré tout, s'arrêter et le signaler plutôt que de déplacer des définitions au jugé.**

- [ ] **Étape 7 : écrire `app/agent/facts.py`**
