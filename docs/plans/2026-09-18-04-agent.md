...
def test_every_fact_a_builder_reads_exists_in_the_catalogue():
    # Une faute de frappe dans un identifiant de fait ferait échouer le
    # constructeur en silence, pour toujours.
    import inspect

    import app.agent.assumptions as module

    source = inspect.getsource(module)
    catalogue = load_catalogue().facts
    import re

    for fact_id in set(re.findall(r'"([a-z_]+)"\)', source)):
        if fact_id in {"prix_moyen_unite", "volume_ventes_an1"} or "_" in fact_id:
            if fact_id not in catalogue and fact_id not in {"a", "facts"}:
                assert fact_id in catalogue, f"identifiant de fait inconnu : {fact_id}"
```
