...
- [ ] **Étape 4 : la relecture qui demande une réécriture**

Aucun test n'emprunte cette branche : toutes les reprises répondent `accept`.
Une régression dans le chemin de réécriture — des `revisions` non remises à
zéro, des problèmes non repris du retour de l'utilisateur — passerait donc
inaperçue. Dans `backend/tests/test_graph.py` :
...
## Tâche 7 ter : les faits utiles ne sont jamais demandés

**Constat de fin de plan, chiffré, et décision du propriétaire du produit : corriger avant la fusion.**

**Fichiers :**
- Modifier : `backend/app/agent/nodes.py`, `backend/tests/test_nodes.py`, `backend/tests/test_graph.py`

**Le manque.** Sur les seize faits que la couche d'hypothèses lit, **sept ne sont déclarés que comme `faits_utiles`** : `emprunt_montant`, `emprunt_duree`, `aides_subventions`, `tresorerie_securite`, `budget_developpement`, `taille_marche_tam`, `taille_marche_sam`. Or `missing_required_facts` ne regarde que `faits_requis`, et c'est la seule source des questions posées. Aucun fait utile n'est donc jamais demandé — ni en test, ni en production. Six calculs sur dix ne peuvent pas tourner, et un business plan sort avec quatre tableaux au lieu de dix.

L'en-tête du catalogue décrit pourtant la règle depuis le début :

> Un fait est soit REQUIS par une section — la section ne peut pas être écrite sans lui, il est donc toujours demandé —, soit UTILE — il est demandé si le lot de questions de la section n'est pas déjà plein.

Cette seconde moitié n'a jamais été écrite.

**La règle, et sa mesure.** Un lot se forme quand une section manque d'un fait requis ; les faits utiles remplissent les places restantes. La taille du lot est **six** : simulé sur le plan complet du business plan, ce chiffre récupère les sept faits visés en quarante-sept questions réparties sur quinze sections, soit environ trois par section. Assez pour servir, trop peu pour ressembler à un interrogatoire.

Une conséquence à assumer : une section dont tous les faits requis sont déjà connus ne forme aucun lot, et ses faits utiles restent sans réponse. C'est la lecture littérale de la règle — le lot existe parce qu'il y a des questions à poser — et la simulation montre qu'elle suffit.

- [ ] **Étape 1 : écrire les tests**

Dans `backend/tests/test_nodes.py` :
...
Et dans `backend/tests/test_graph.py`, resserrer l'assertion de l'exécution du business plan :
...
- [ ] **Étape 2 : écrire la règle**

Dans `backend/app/agent/nodes.py`, à côté des deux plafonds existants :
...
et, à côté de `missing_required_facts` :
...
`formulate_questions` demande désormais le lot et non les seuls faits requis :

```python
        prompts.questions_prompt(section, question_batch(state), _catalogue()),
```

`route_after_gaps` ne change pas : c'est toujours l'absence d'un fait **requis** qui déclenche un tour de questions.

- [ ] **Étape 3 : le simulé cesse de tronquer les lots à deux**

La règle du lot ne sert à rien si le modèle simulé n'en rend que deux éléments.
`_value_for` fabrique exactement deux entrées pour toute liste, quel que soit
le nombre de faits que le prompt propose : un lot de six revient donc avec deux
questions, et deux calculs sur dix restent sans données — le tableau de marché
et le compte de résultat, soit les deux plus importants d'un business plan.

Le prompt de questions dit pourtant au modèle : « reformule ces questions, sans
en ajouter ni en retirer ». Un vrai modèle en rend donc autant qu'on lui en
offre. Dans `backend/app/llm/fake.py`, la branche des listes :
...
Et un test dans `backend/tests/test_fake.py` :
...
Run : `uv run pytest tests/test_nodes.py tests/test_graph.py tests/test_fake.py -v`, puis la suite entière.

**Rapporter le nombre de tableaux que l'exécution du business plan produit désormais, et lesquels des dix calculs tournent.** C'est la mesure qui dit si la règle a servi.

```bash
git add backend/app/agent/nodes.py backend/app/llm/fake.py \
        backend/tests/test_nodes.py backend/tests/test_graph.py backend/tests/test_fake.py
git commit -m "feat(agent): le lot de questions se complète avec les faits utiles"
```
