...
**Un export, un préfixe.** Un horodatage UTC à la microseconde est tiré une seule fois par export, et les quatre fichiers sont déposés sous `{project_id}/{horodatage}/{document}.{format}`. Les fichiers sont écrits d'abord ; les lignes de la table `exports` ne sont enregistrées qu'ensuite, en une transaction unique. Comme `GET /projects/{id}/exports` ne sert que les chemins que la base nomme, un dépôt interrompu en cours de route (panne réseau sur le troisième fichier, par exemple) n'est jamais visible : les lignes continuent de pointer sur l'export précédent, entier et cohérent — mêmes liens, mêmes drapeaux `brouillon`/`fidele` qu'avant la tentative. L'ancien préfixe n'est supprimé qu'après que la transaction a réussi ; un échec de cette suppression est journalisé et n'empêche ni l'export de réussir ni ses lignes d'être enregistrées — un fichier orphelin coûte quelques centaines de kilo-octets, pas une erreur. Une ligne écrite avant ce correctif porte un chemin plat (`{project_id}/{document}.{format}`, sans dossier horodaté) ; pour un projet qui ne réexporte jamais, ces fichiers restent simplement en place. Pour un projet qui réexporte, ce chemin plat n'est jamais traité comme un préfixe à supprimer — il désignerait l'ancêtre du nouveau dossier horodaté, pas un dossier à lui — et reste donc orphelin lui aussi : même prix qu'un dépôt interrompu, payé une fois. Symétriquement, un export interrompu APRÈS le dépôt mais AVANT que la transaction n'enregistre les lignes (panne réseau, base injoignable) laisse ses fichiers neufs au stockage sans qu'aucune ligne ne les nomme : rien ne les sert, rien ne les supprimera. C'est le prix de l'invisibilité voulue par ce correctif, et il se dit plutôt que de se découvrir en production.
